        ...
        return (
            <div className={c.users}>
                <div className={c.pages}>
                    {
                       pages.map( p => <span onClick={(e) => {this.onPageChange(p)} } className={this.props.currentPage === p && c.activePage}>{p}</span>) 
                    }
                </div>
                {
                    this.props.users.map( (u) => <User key={u.id} id={u.id} isFollowed={u.followed} name={u.name} status={u.status} img_src={u.photos.small != null ? u.photos.small : defaultUserPhoto} submitFollowing={this.props.submitFollowing} submitUnfollowing={this.props.submitUnfollowing} />)
                }
            </div>
        )
    }
